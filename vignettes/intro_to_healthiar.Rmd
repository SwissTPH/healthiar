...
The Python wrapper is an implementation of the [`rpy2`
...
any function available in the R package. To illustrate a typical workflow, 
let's replicate the above example of a call to `attribute_health()` 
...
`spatial` contains functions to read geographic vector or raster data in way that
is compatible with the `healthiar` function `prepare_exposure()`:
