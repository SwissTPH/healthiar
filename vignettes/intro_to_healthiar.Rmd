...
Goal: To quantify attributable health impacts *stratified by a sub-group different to age and sex, e.g. education level*.
...
Goal: To quantify attributable health impacts *stratified by age, sex and additional sub-group e.g. education level*.
...
Goal: E.g. To quantify health impacts using `attribute_health`in an scenario B very similar to a previous scenario A.
...
Goal: To estimate the health impact that is theoretically attributable to the difference in a social indicator of the population exposed (e.g. degree of deprivation). 

The function `socialize()` can be used for this aim either entering a) the whole output of `attribute_health()` or b) directly the health impact in the `socialize()` argument `impact`.  
