...
Please note that the PIF comparison approach assumes same baseline health data 
...
Therefore, the delta approach is suited for comparison of a situation now with a situation in the future.
...
### Methodology
Health impacts can be monetized by valuating them (giving them an econmic value)
and by applying discounting and/or inflation. 
For this purpose, you can use `monetize()`. 

If you just need the discount factor or the inflation factor, 
you can alternatively call `get_discount_factor()` or `get_inflation_factor()`. 

If you just need the discounted health impacts (to be valuated in a further step), 
you can alternatively call `discount()`. 
...
To estimate the benefit of a health policy via cost-benefit analysis (CBA).

### Methodology

The cost-benefit analysis compares the cost and the benefit of an investment.  
...
It provides as output three types of indicators 
based on the following equations [@Boardman2018_book]:
