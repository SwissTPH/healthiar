...
`healthiar`, e.g., `attribute_lifetable()`, `compare()`, `socialize()`
...
1.  Install the `healthiar` wrapper module using pip... [requires the
    healthiar wrapper to be packaged on pypi.org -\> work in progress]
...
A correct usage requires a conversion of the input and output data using
these functions:
...
each relevant Python class, and the function used for conversion in case
...
| `list`, `tuple` | `vector` | `py_to_r()` |
| `pandas.DataFrame, pandas.Series` | `data.frame` | `py_to_r()` |
| `function` | `function` | *use a decorator (see below)* |

To illustrate a typical workflow, let's replicate the above example of a
call to `attribute_health()` for multiple geographic units.
...
directly to another `healthiar` function, such as `compare()`. To
convert the result to a regular Python object, pass it to the
`r_to_py()` function:
