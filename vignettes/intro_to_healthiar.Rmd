...
There is extensive literature on health impact quantification using the relative risk approach [e.g., Pozzer2023_gh; @Soares2020_report; @GBD2020_tl; @Steenland2006-e; @WHO2003_report]. 
...
$$PAF = \frac{\int RRexp(x)PE(x)dx - 1}{\int RRexp(x)PE(x)dx}$$
...
* $RRexp(x)$ = relative risk at exposure level compared to reference

##### Simplified for categorical exposure distribution
If exposure is categorical, the integrals are converted to sums:

$$PAF = \frac{\sum RRexp_i \times PE_i - 1}{\sum RRexp_i \times PE_i}$$

Alternatively, an equivalent form is:

$$PAF = \frac{\sum PE_i(RRexp_i - 1)}{\sum PE_i(RRexp_i - 1) + 1}$$

##### Simplified for single exposure value 
The single exposure value corresponds to the population weighted mean concentration.

$$PAF = \frac{RRexp - 1}{RRexp}$$
#### Scaling relative risk
The relative risk at exposure level (RRexp) is called `rr_at_exp` in `healthiar` and is part of the output of `attribute_health()` and `attribute_lifetable()`. RRexp can also be calculated using `get_risk()`.

But how to get this relative risk at exposure level (RRexp)? This is normally different to the one published in the epidemiological literature (RRlit). The equations used for scaling relative risk depend on the chosen exposure-response function shapes:

- linear
$$RRexp = 1 + \frac{RRlit - 1}{RRlit\_Increment} \cdot (exp - cutoff)$$

- log-linear [@Pozzer2023_gh]
$$RRexp = e^{\frac{\log(RRlit)}{RRlit\_increment} \cdot (exp - cutoff)}$$

- log-log [@Pozzer2023_gh]
$$RRexp = \left( \frac{exp + 1}{cutoff + 1} \right)^{\frac{\log(RRlit)}{\log(RRlit\_Increment + cutoff + 1) - \log(cutoff + 1)}}$$

- linear-log [@Pozzer2023_gh]
$$RRexp = 1 + \frac{\log(RRlit - 1)}{\log(RRlit\_increment + cutoff + 1) - \log(cutoff + 1)} \cdot \frac{\log(exp + 1)}{\log(cutoff + 1)}$$
...
To summarize uncertainty of attributable health impacts (i.e. to get a single confidence interval instead of many combinations) by using a Monte Carlo simulation.
...
#### General concept
The life table methodology of `attribute_lifetable()` follows that of the WHO tool AirQ+ [@WHO2020_report], 
which is described in more detail by @Miller2003_jech. 
...
* $RRexp(x)$ = relative risk at exposure level compared to the reference level
...
* $RRexp_i$ = relative risk for exposure category level $i$ compared to the reference level
...
* $RRexp$ = relative risk associated with the population weighted mean exposure
* $RRexp{alt}$ = relative risk associated with the population weighted mean for the alternative exposure scenario
