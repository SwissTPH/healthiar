...
How to get the relative risk at exposure level (RRexp). This is normally different to the one published in the epidemiological literature. The equations used for scaling relative risk depend on the chosen exposure-response function shapes:

- linear
$$rr\_at\_exp = 1 + \frac{rr - 1}{rr\_increment} \cdot (exp - cutoff)$$

- log-linear [@Pozzer2023_gh]
$$rr\_at\_exp = e^{\frac{\log(rr)}{rr\_increment} \cdot (exp - cutoff)}$$

- log-log [@Pozzer2023_gh]
$$rr\_at\_exp = \left( \frac{exp + 1}{cutoff + 1} \right)^{\frac{\log(rr)}{\log(rr\_increment + cutoff + 1) - \log(cutoff + 1)}}$$

- linear-log [@Pozzer2023_gh]
$$rr\_at\_exp = 1 + \frac{\log(rr - 1)}{\log(rr\_increment + cutoff + 1) - \log(cutoff + 1)} \cdot \frac{\log(exp + 1)}{\log(cutoff + 1)}$$
