...
load("../varia/input/data/input_data_for_testing_Rpackage.RData")
```
...
remotes::install_github(
  ...
  ref = "568-add-example-descriptions-to-vignette", # By default "HEAD"; name of branch to install package from as a string
  force = TRUE,
...
# devtools::build(binary = TRUE, path = "../varia/Workshops_and_demos/2025_04_10_UBD_policy") # Save current state of package as .zip file (e.g. to directly test changes in functions)
# install.packages("../varia/input/healthiar_0.0.0.1.zip", repos=NULL, type='source') # Modify file path

## Install package from .tar.gz file
# devtools::build(path = "../varia/input/") # Save current state of package as tar.gz file (e.g. to directly test changes in functions)
# install.packages("../varia/input/healthiar_0.0.0.1.tar.gz", repos=NULL, type='source') # Modify file path
...
round(as.numeric(bestcost_noise_ihd_expDist_iteration[["health_detailed"]][["results_raw"]] |> filter(erf_ci == "central") |> filter(exp_ci == "central") |> pull(impact_rounded)), digits = 0) 
...
bestcost_noise_ihd_expDist_iteration[["health_detailed"]][["results_raw"]] |> filter(erf_ci == "central") |> filter(exp_ci == "central") |> pull(pop_fraction)
...
check_general(bestcost_cost_discount_single_value[["monetization_main"]]$monetized_impact_rounded,
              11074) 
## Result according to ChatGPT on 10 March 2025 
