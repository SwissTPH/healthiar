...
pathways_ar <- pathways_ar |> 
  left_join(x = _, y = core_ar, by = c("erf", "exp", "cutoff", "varuncer", "iteration", "multiexp")) |> 
  mutate(core_pathway = ifelse(is.na(core_pathway), "FALSE", core_pathway)) |> 
  mutate(implemented = "FALSE")
...
pathways_cba_direct <- list(
  ...
  discount_overtime = c("all_years", "last_year") # all_years is default ; only applicable if discount_appr is "direct" 
)
...
pathways_cba_indirect <- list(
  ...
  discount_rate_cost = c("TRUE", "FALSE"),
  discount_shape = c("exp", "hyp_harvey", "hyp_mazur"),
  discount_overtime = c("NA") # all_years is default ; only applicable if discount_appr is "direct" 
)
...
core_cba_direct <- list(
  ...
  discount_rate_cost = c("TRUE", "FALSE"),
  discount_shape = c("exp", "hyp_harvey", "hyp_mazur"),
  discount_overtime = c("all_years") # all_years is default ; only applicable if discount_appr is "direct" 
)
...
pathways_cba <- pathways_cba |> 
  left_join(x = _, y = core_cba, by = c("discount_appr", "discount_shape", "discount_overtime", "discount_rate_benefit", "discount_rate_cost")) |> 
  mutate(core_pathway = ifelse(is.na(core_pathway), "FALSE", core_pathway)) |> 
  mutate(implemented = "FALSE")

# * Document existing pathways #################################################
file_path <- "../healthiar/tests/testthat/test-include_cba.R"
file_content <- readLines(file_path) # String vector where each element is all code from a line
...
existing_tests <- tibble(
  ...
  discount_overtime = rep(NA_character_, num_rows),
  implemented = rep("FALSE", num_rows)
)
...
for (i in seq_along(test_names_with_pw)) {
  ...
  existing_tests$discount_overtime[i] <- str_extract(strings[str_detect(strings, "\\|discount_overtime_")], "(?<=\\|discount_overtime_)[^\\|]+")
...
## Merge the pathways tibble and the existing_tests tibble
pathways_cba <- pathways_cba %>%
  left_join(existing_tests, by = c("discount_appr", "discount_shape", "discount_overtime", "discount_rate_benefit", "discount_rate_cost"),
            suffix = c("", ".existing")) %>%
  mutate(implemented = coalesce(implemented.existing, implemented)) %>%
  select(-implemented.existing) |> 
  distinct() # Removes duplicate rows (rows that have the same pathway ID)
...
core_monetization <- list(
  discount_appr = c("direct", "indirect"), # direct is the default
  discount_shape = c("exp", "hyp_harvey", "hyp_mazur"),
  discount_overtime = c("all_years"), # "last_year" is a nice to have
  discount_rate = c("TRUE", "FALSE")
)
...
pathways_monetization <- pathways_monetization |> 
  left_join(x = _, y = core_monetization, by = c("discount_appr", "discount_shape", "discount_overtime", "discount_rate")) |> 
  mutate(core_pathway = ifelse(is.na(core_pathway), "FALSE", core_pathway)) |> 
  mutate(implemented = "FALSE")
...
## Merge the pathways tibble and the existing_tests tibble
pathways_monetization <- pathways_monetization %>%
  left_join(existing_tests, by = c("discount_appr", "discount_shape", "discount_overtime", "discount_rate"),
            suffix = c("", ".existing")) %>%
  mutate(implemented = coalesce(implemented.existing, implemented)) %>%
  select(-implemented.existing)|> 
  distinct() # Removes duplicate rows (rows that have the same pathway ID)
...
file_path <- "../healthiar/tests/testthat/test-include_summary_uncertainty.R"
file_content <- readLines(file_path) # String vector where each element is all code from a line
...
core_cba <- pathways_cba |> 
  ...
  mutate(results_the_same = NA_character_) |> 
  filter(core_pathway == "TRUE") |> 
  arrange(discount_appr, desc(discount_rate_benefit), desc(discount_rate_cost), discount_shape, discount_overtime, )
...
core_monetization <- pathways_monetization |> 
  ...
  mutate(results_the_same = NA_character_) |> 
  filter(core_pathway == "TRUE") |> 
  arrange(discount_appr, desc(discount_rate), discount_shape, discount_overtime, )
