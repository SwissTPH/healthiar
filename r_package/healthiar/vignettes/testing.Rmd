...
Please note that there might not be a helper function to extract main / detailed results for all calculation pathways. Please refer to the *intro_to_healthiar* vignette for guidance on how to extract main / detailed results of an attribute output.
...
In this example the `pathway_id` is *pathway_rr\|erf_log_lin\|exp_single\|cutoff_TRUE\|varuncer_FALSE\|iteration_FALSE\|multiexp_FALSE\|*
...
## Pathway ID: 
## pathway_rr|erf_log_lin|exp_single|cutoff_TRUE|varuncer_FALSE|iteration_FALSE|multiexp_FALSE|
```
...
healthiar::monetize(
  ...
  discount_shape = NULL,
  discount_years = 1,
  discount_overtime = "all_years", # only applicable if approach_discount is "direct" 
  inflation = NULL
)
...
testthat::test_that("results correct [pathway_id]", {
  ...
  testthat::expect_equal(
    ...
    object =
      ...
      healthiar::helper_extract_main_monetization_results(),
    ##  RESULT(S) FROM THE COMPARISON ASSESSMENT YOU SELECTED
    expected =
      c( )
    )
...
Note that to test the `include_social` function you a social indicator for each of the geo units under analysis in addition to the usual data needed for an iteration of `attribute`.
