...
2.  an extended template

in both cases for a relative risk pathway.

### Example basic function template

This basic template consists of

-   pathway ID (of the specific pathway you want to test)

-   optionally: some code to load in input data
...
## RESULT(S): List here the results of the comparison assessment you selected
...
### Example extended function template

The extended function template contains the same elements as the basic function template plus some additional ones. In addition to the elements of the basic function template the extended one contains

-   a `testthat::expect_equal` call, which encapsulates two main arguments
...
The `testthat::expect_equal` enables an automatic comparison of the `attribute_...` results and of the comparison assessment results.
...
## 3. OPTIONAL: Check whether results of healthiar and your chosen assessment are the same

Note: even though this step is option, if you have advanced knowledge of R we are very grateful if you can complete it.

Now you check whether the results of the `healthiar` function and your chosen comparison assessment are the same by running the `test_that` function.
...
```{r template monetization complete, include=TRUE, echo=TRUE, eval=FALSE}
testthat::test_that("results correct [pathway_id]", {
...
```{r template iteration rr, include=TRUE, echo=TRUE, eval=FALSE}
testthat::test_that("results correct [pathway_id]", {
...
```{r template social analysis, include=TRUE, echo=TRUE, eval=FALSE}
