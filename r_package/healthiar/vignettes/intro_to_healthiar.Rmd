...
-   `health_detailed` detailed results and additional info about the assessment
    ...
    -   `results_summed_across_ar:exp_cat` contains results per geo unit after aggregating across exposure categories in absolute risk
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_iteration_ar$health_detailed$results_summed_across_ar_exp_cat
```
...
```{r eval=TRUE, include=TRUE, echo=FALSE}
results_daly$health_main |> 
  dplyr::select(impact_rounded_yll, impact_rounded_yll, impact_rounded) |> 
  knitr::kable()
```
