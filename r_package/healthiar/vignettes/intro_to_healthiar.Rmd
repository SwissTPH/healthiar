...
-   `health_detailed` detailed results and additional info about the assessment
    ...
    -   `results_by_`: contains results stratified by variables such as exposure category (only absolute risk), sex, age group and geo ids. These tables are only shown if the variables have more than one category (except results_by_geo_id_disaggegated, which is always shown).  

    -   `results_summed_with_all_ci` contains results summing imapct of across all the above variables but still keeping all uncertainties (confidence intervals)
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_iteration_ar$health_detailed$results_by_geo_id_disaggregated
```
