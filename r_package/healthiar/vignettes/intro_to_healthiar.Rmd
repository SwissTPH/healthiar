...
-   `health_detailed` detailed results and additional info about the assessment
    ...
    -   `results_summed_across_exp_cat` contains results per geo unit after aggregating across exposure categories

    -   `results_summed_across_sex` & `results_summed_across_age` contains results aggregated by sex and age groups, respectively
...
results_iteration_ar <- attribute_health( 
    ...
    geo_id_disaggregated = data$region,
    approach_risk = "absolute_risk",
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_iteration_ar$health_detailed$results_summed_across_exp_cat
```
...
output_stratified <- output_attribute$health_detailed$results_summed_across_exp_cat |>
...
```{r}
results_pm_yll$health_detailed$results_raw$impact_by_year_nested$female_a_central_central_central_central
```
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_yll$health_detailed$results_raw$deaths_by_age_and_year_nested$female_a_central_central_central_central
```
...
results_pm_yll$health_detailed$results_raw$deaths_by_age_and_year_nested$female_a_central_central_central_central |> 
  ...
  dplyr::slice(
    ( nrow( results_pm_yll$health_detailed$results_raw$deaths_by_age_and_year_nested$female_a_central_central_central_central ) -8 ) : 
      nrow( results_pm_yll$health_detailed$results_raw$deaths_by_age_and_year_nested$female_a_central_central_central_central )
    ) |> 
  knitr::kable()
```
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_yll$health_detailed$results_raw$yll_by_age_and_year_nested$female_a_central_central_central_central
```

```{r echo=FALSE}
results_pm_yll$health_detailed$results_raw$yll_by_age_and_year_nested$female_a_central_central_central_central |>    
  dplyr::select(age_start, impact_2019, impact_2020, impact_2021, impact_2022) |>    
  dplyr::slice(
    ( nrow(results_pm_yll$health_detailed$results_raw$yll_by_age_and_year_nested$female_a_central_central_central_central ) -8 ) : 
      nrow(results_pm_yll$health_detailed$results_raw$yll_by_age_and_year_nested$female_a_central_central_central_central )
    ) |>    
  knitr::kable()
```
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_yll$health_detailed$results_raw$projection_if_exposed_nested$female_a_central_central_central_central
```
...
results_pm_yll$health_detailed$results_raw$projection_if_exposed_nested$female_a_central_central_central_central |>    
  ...
  dplyr::slice(
    ( nrow(results_pm_yll$health_detailed$results_raw$projection_if_exposed_nested$female_a_central_central_central_central ) -8 ) : 
      nrow(results_pm_yll$health_detailed$results_raw$projection_if_exposed_nested$female_a_central_central_central_central )
    ) |>    
  knitr::kable()
```
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_yll$health_detailed$results_raw$projection_if_unexposed_nested$female_a_central_central_central_central
```
...
results_pm_yll$health_detailed$results_raw$projection_if_unexposed_nested$female_a_central_central_central_central |>    
  dplyr::select(age_start, population_2019, population_2020, population_2021, population_2022) |>    
  dplyr::slice(
    ( nrow( results_pm_yll$health_detailed$results_raw$projection_if_unexposed_nested$female_a_central_central_central_central ) -8 ) : 
      nrow( results_pm_yll$health_detailed$results_raw$projection_if_unexposed_nested$female_a_central_central_central_central )
    ) |>    
  knitr::kable()
```
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_deaths$health_detailed$results_raw$impact_by_year_nested$female_a_central_central_central_central
```

```{r eval=TRUE, include=TRUE, echo=FALSE}
results_pm_deaths$health_detailed$results_raw$impact_by_year_nested$female_a_central_central_central_central |> 
  dplyr::slice( ( nrow( results_pm_deaths$health_detailed$results_raw$impact_by_year_nested$female_a_central_central_central_central )-8 ) : 
           nrow( results_pm_deaths$health_detailed$results_raw$impact_by_year_nested$female_a_central_central_central_central )) |> 
  knitr::kable()
```
