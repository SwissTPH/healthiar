...
Some of these example data will be used in the examples in this vignette.

```{r, eval=FALSE, echo=FALSE, include=TRUE}
# NOTE 2025-06-13 AL: seemingly there is only one variable "example_data now"
# package_data <- data(package = "healthiar")[["results"]] |> 
#   as_tibble() |> 
#   select(Item) |> 
#   # filter(str_detect(Item, "exdat")) |>
#   filter(str_detect(Item, "example_data")) |>
...
We call the `attribute_health` with input data from the `healthiar` example data. Note that we can easily provide input data to the function argument using the `$` operator.
...
-   `health_detailed` detailed results and additional info about the assessment
    ...
    - `impact_raw` tibble that contains the detailed results
    ...
    - ONLY FOR ITERATION: `impact_agg_geo` tibble that contains detailed aggregated results
    
*NOTE*: the output of `attribute_lifetable()` contains additional results that are specific to life table calculations.
...
It is a tibble of 3 rows and 23 columns. Let's zoom in on some relevant aspects
...
(*NOTE*: only `r length(1:9)` of the `r length(results_pm_copd$health_detailed$impact_raw$impact)` possible combinations are displayed due to space constraints.)
...
results_iteration[["health_main"]] |> 
  ...
  knitr::kable()
```
Main (= *aggregated*). The cumulative / summed number of stroke cases attributable to PM2.5 exposure in the 5 geo units is `r sum(results_iteration[["health_main"]]$impact_rounded)` (using a relative risk of 1.369).
...
```
*NOTE*: the length of the input vectors fed to `geo_id_disaggregated`, `exp_central`, `pop_exp` must match and must be (number of geo units) x (number of exposure categories) = 2 x 5 = **10**, because we have 2 lower level geo units (`"rural"` and `"urban"`) and `r length(exdat_noise_ha$exposure_mean)` exposure categories.
...
results_pm_copd_summarized |> 
  ...
  dplyr::slice_head() |> 
  dplyr::pull(impact_main)
```
We see that the detailed results contain the complete `n_sim` simulation (one row for each simulation), which are saved in nested tibbles in the column `impact_main`.
```{r, echo=FALSE, eval=TRUE, include=FALSE}
...
```{r eval=FALSE}
results_pm_yll$health_detailed$impact_raw$yll_nest$total_a_central_central_central
```

```{r, echo=FALSE}
results_pm_yll$health_detailed$impact_raw$yll_nest$total_a_central_central_central |> 
  select(age, population_2019, population_2020, population_2021) |> 
  slice( ( nrow(results_pm_yll$health_detailed$impact_raw$yll_nest$total_a_central_central_central)-8 ) : 
           nrow(results_pm_yll$health_detailed$impact_raw$yll_nest$total_a_central_central_central)) |> 
  knitr::kable()
```
...
- `results_pm_yll[["health_detailed"]][["impact_raw"]][["premature_deaths_nest"]][["total_a_central_central_central"]] |> select(age, age_end, deaths_2019)` pre-mature deaths in the year of exposure (here: 2019) attributable to exposure
...
```{r eval=FALSE}
results_pm_deaths$health_detailed$impact_raw$premature_deaths_nest$total_a_central_central_central
```

```{r, echo=FALSE}
results_pm_deaths$health_detailed$impact_raw$premature_deaths_nest$total_a_central_central_central |> 
  slice( ( nrow(results_pm_deaths$health_detailed$impact_raw$premature_deaths_nest$total_a_central_central_central)-8 ) : 
           nrow(results_pm_deaths$health_detailed$impact_raw$premature_deaths_nest$total_a_central_central_central)) |> 
  knitr::kable()
```
```{r, echo=FALSE, eval=TRUE, include=FALSE}
