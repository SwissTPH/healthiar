...
results_pm_yll$health_detailed$results_raw |> 
  ...
  dplyr::pull(impact_by_age_and_year_nested) |>
  purrr::pluck(1)
```
...
results_pm_yll$health_detailed$results_raw|> 
  dplyr::filter(sex == "female") |>
  dplyr::pull(projection_if_exposed_by_age_and_year_nested) |>
  purrr::pluck(1)
```
...
results_pm_yll$health_detailed$results_raw|> 
  dplyr::filter(sex == "female") |>
  dplyr::pull(projection_if_unexposed_by_age_and_year_nested) |>
  purrr::pluck(1)
```
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_deaths$health_detailed$results_raw|> 
  dplyr::filter(sex == "female") |>
  dplyr::pull(projection_if_unexposed_by_age_and_year_nested) |>
  purrr::pluck(1)
```
