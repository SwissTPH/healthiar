...
```{r}
results_pm_yll$health_detailed$results_raw$impact_by_year$female_a_central_central_central_central
```
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_yll$health_detailed$interim_results |> 
...
```{r eval=FALSE, include=FALSE, echo=TRUE}
results_pm_deaths$health_detailed$interim_results|> 
  dplyr::filter(sex == "female") |>
  dplyr::pull(projection_if_unexposed_by_age_and_year) |>
  purrr::pluck(1)
```
