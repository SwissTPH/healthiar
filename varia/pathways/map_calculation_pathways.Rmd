...
pathways_rr <- list(
  ...
  strat = c("TRUE", "FALSE"), # stratification (by both age_group and sex arguments)
  yld = c("TRUE", "FALSE") # years lived with disability (i.e. both dw_... and duration_... arguments)
  # pop = c("TRUE", "FALSE") # population; TBD whether we'll introduce this later
)
...
pathways_rr <- pathways_rr |> 
  left_join(x = _, y = core_rr, by = c("erf", "exp", "iteration", "uncertainty", "strat", "yld")) |> 
...
file_content <- file_content[!grepl("^\\s*#", file_content)] # Filter out commented-out lines
...
existing_tests <- tibble(
  ...
  uncertainty = rep(NA_character_, n_row),
  strat = rep(NA_character_, n_row),
  yld = rep(NA_character_, n_row),
  implemented = rep(FALSE, n_row)
)

## Extract values and assign them directly to the tibble

## NOTE 2025-11-04: if there's an incomplete pathway ID within the list "existing_tests" the loop below will fail
## --> make sure that all pathway ID's in the script "test-attribute_health.R" are extended with the new parts
for (i in seq_along(test_names_with_pw)) {

  strings <- test_names_with_pw[[i]] # Current list item for which values will be extracted
  ...
  existing_tests$uncertainty[i] <- str_extract(strings[str_detect(strings, "\\|uncertainty_")], "(?<=\\|uncertainty_)[^\\|]+")
  existing_tests$strat[i] <- str_extract(strings[str_detect(strings, "\\|strat_")], "(?<=\\|strat_)[^\\|]+")
  existing_tests$yld[i] <- str_extract(strings[str_detect(strings, "\\|yld_")], "(?<=\\|yld_)[^\\|]+")
...
pathways_rr <- pathways_rr %>%
  left_join(existing_tests, by = c("erf", "exp", "iteration", "uncertainty", "strat", "yld"),
            suffix = c("", ".existing")) %>%
...
pathways_monetization <- pathways_monetization |> 
  left_join(x = _, y = core_monetization, by = c("discount_shape", "discount_rate", "inflation_rate")) |> 
  mutate(core_pathway = ifelse(is.na(core_pathway), "FALSE", core_pathway)) |> 
  mutate(implemented = "FALSE")
...
existing_tests <- tibble(
  ...
  inflation_rate = rep(NA_character_, n_row),
  implemented = rep("FALSE", n_row)
)
...
for (i in seq_along(test_names_with_pw)) {
  ...
  existing_tests$inflation_rate[i] <- str_extract(strings[str_detect(strings, "\\|inflation_rate_")], "(?<=\\|inflation_rate_)[^\\|]+")
...
## Merge the pathways tibble and the existing_tests tibble
pathways_monetization <- pathways_monetization %>%
  left_join(existing_tests, by = c("discount_shape", "discount_rate", "inflation_rate"),
            suffix = c("", ".existing")) %>%
  mutate(implemented = coalesce(implemented.existing, implemented)) %>%
  select(-implemented.existing)|> 
  distinct() # Removes duplicate rows (rows that have the same pathway ID)
...
pathways_monetization <- pathways_monetization |> 
  relocate(implemented) |> 
  relocate(core_pathway, .after = implemented) |> 
  arrange(desc(discount_rate), discount_shape, inflation_rate)
...
pathways_multiexposure <- list(
  approach_multiexposure = c("additive", "multiplicative", "combined")
)
...
pathways_multiexposure <- pathways_multiexposure |> 
  left_join(x = _, y = core_multiexposure, by = c("approach_multiexposure")) |> 
  mutate(core_pathway = ifelse(is.na(core_pathway), "FALSE", core_pathway)) |> 
  mutate(implemented = "FALSE")
...
n_row <- length(test_names_with_pw)
existing_tests <- tibble(
  approach_multiexposure = rep(NA_character_, n_row),
  implemented = rep("FALSE", n_row)
)
...
for (i in seq_along(test_names_with_pw)) {
  ...
  # Extract values for each column
  existing_tests$approach_multiexposure[i] <- str_extract(strings[str_detect(strings, "\\|approach_multiexposure_")], "(?<=\\|approach_multiexposure_)[^\\|]+")
...
## Merge the pathways tibble and the existing_tests tibble
pathways_multiexposure <- pathways_multiexposure %>%
  left_join(existing_tests, by = c("approach_multiexposure"),
            suffix = c("", ".existing")) %>%
  mutate(implemented = coalesce(implemented.existing, implemented)) %>%
  select(-implemented.existing) |> 
  distinct() # Removes duplicate rows (rows that have the same pathway ID)
...
pathways_multiexposure <- pathways_multiexposure |> 
  relocate(implemented) |> 
  relocate(core_pathway, .after = implemented) |> 
  arrange(desc(implemented), desc(core_pathway), approach_multiexposure)
...
## Create Excel for overall progress
openxlsx::write.xlsx(progress_overall, "varia/pathways/progress_overall.xlsx")
```


# Create Excel
                                                                     
```{r Create Excel with all pathways for Teams folder, eval=FALSE, include=FALSE}
...
##
# openxlsx::write.xlsx(pathways_list, "varia/pathways/overview_testing.xlsx",
#                      headerStyle = hs,
