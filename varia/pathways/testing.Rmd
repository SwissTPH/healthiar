...
library(tidyr)
library(knitr)
# library(readxl) # NOTE 2025-09-09 AL: this line has been commented out to remove the package dependency on readxl
library(remotes)
...
-   Preferably by opening a GitHub issue by going on this
    [link](https://github.com/SwissTPH/healthiar/issues) and clicking the
...
```{r install package, include=FALSE, echo=TRUE, eval=FALSE}
remotes::install_github(
  repo = "SwissTPH/healthiar", 
  ref = "HEAD", 
...
```{r inspect excel file, include=FALSE, eval=TRUE}
...
NOTE 2025-09-09 AL: this chunk has been commented out to remove the package dependency on readxl
...
```{r filled out template extended, include=TRUE, echo=TRUE, eval=FALSE}
testthat::test_that("results correct |pathway_rr|erf_log_lin|exp_single|iteration_FALSE|", {
...
```{r uncertainty example part 2 extract results, eval=TRUE}
...
```{r test uncertainty rr iteration, eval=TRUE, include=FALSE}
testthat::test_that("results correct [pathway_id]", {
...
-   *iteration* specifies whether the assessment is done for more than one geo
    ...
    -   if yes –\> TRUE: the iteration function calls require two additional
        ...
        to additionally specify the `geo_id_micro` (e.g. "Brussels",
        "Antwerp", ...) and `geo_id_macro` (e.g. "Belgium") arguments.
...
-   *inflation_rate* specifies whether annual inflation (increase of prices) is
    applied
  ...
  testthat::expect_equal(
    ...
    object =
      healthiar::cba(
        ...
        discount_rate_cost = ,
        n_years_benefit = 1,
        n_years_cost = 1)$cba_main$net_benefit_rounded,
    ##  RESULT(S) FROM THE COMPARISON ASSESSMENT YOU SELECTED
    expected =
      c( )
    )
...
healthiar::socialize(
  ...
  geo_id_micro = , # geo IDs of the preparatory iteration call above and this function call must match!
  social_indicator = ,
