  ...
  function(airqplus_export){
    ...
    airqplus_output <- 
      ...
      airqplus_ready <- 
        ...
        dplyr::mutate(dplyr::across(dplyr::starts_with("estimated_attributable_proportion"),
                             readr::parse_number))%>%
        # Divide by 100 because the columns were a percentage originally (e.g. 5% = 0.05)
         dplyr::mutate(dplyr::across(dplyr::starts_with("estimated_attributable_proportion"),
                              ~./100)) %>%
        # Convert to numeric
        dplyr::mutate(dplyr::across(-c(analysis_name, type, pollutant, location,
...
digest_airqplus_export_lifetable <- 
  function(airqplus_export){
    ...
    airqplus_export_edited <- 
      airqplus_export %>%
      # Rename variables
      stat::setNames(c("variable", "value")) %>%
    ...
    airqplus_yll <- 
      ...
      dplyr::slice(-1) %>%
      # Pivot wider
      tidyr::pivot_wider(values_from = dplyr::starts_with("value_"),             
                         names_from = "variable")%>%
      # Convert to numeric
      dplyr::mutate(across(-c(section_name, -subsection_name), 
                           as.numeric))
...
check_bestcost <-
  ...
  result_vector_bestcost <- 
    result_list_bestcost[["health_main"]]%>%
    # Set order of rows
  ...
  difference_prop <-
    ifelse(result_vector_bestcost == result_vector_alternative, 
           c(0, 0, 0),
           (result_vector_bestcost - result_vector_alternative)/result_vector_alternative) |>
    round(4)
  difference_percent <-
    paste0(difference_prop*100, "%")
...
check_general <-
  function(a, b){
    diff <- round(a, 4) - round(b, 4)
         # Check difference between bestcost and alternative source
  # First the show the difference between bestcost results and alternative source
  cat(
    paste0("Difference ", paste(diff, collapse = "; "),"\n"))
  # Second show message with colors
  if(all(diff == 0)){
    cat(paste(crayon::cyan("Right. Bestcost and alternative with identical results.")))
    
     } else{
    cat(paste0("best-cost: ", paste(a, collapse = ";")),"\n", 
        paste0("alternative_source: ", paste(b, collapse = "; ")),"\n", 
        crayon::red("Attention! Check! Bestcost and alternative with different results."))}
  }
...
percentage_pm2.5_in_pm10 <- 
  read.csv("varia/archive/input/data/rr_other_rawdata/generic_data.csv", sep = ";") %>% 
...
rr_rawdata <- 
  read.csv("varia/archive/input/data/rr_other_rawdata/rr.csv",
           sep = ";",
           na.strings=c("NA", ""))

# Population data by age (To be used in the life table approach)
population_byAge_rawdata <-
  read.csv2("varia/archive/input/data/population/population_byAge_bySex_CH_2018_2019.csv") %>%
  # Rename columns
...
bhd_rawdata <- 
  read.csv("varia/archive/input/data/rr_other_rawdata/bhd.csv", sep = ";")
...
for(s in sex){
  ...
  lifetable[[s]] <- 
    read.csv2(
      ...
      paste0("varia/archive/input/data/life_tables/life_table_2013_2020_",
             s,
             ".csv"))
}
...
nonNatural_mortality_rawdata<- 
   list(
     male = read.csv2("varia/archive/input/data/mortality/nonNatural_mortality_byAge_2009_2019_male.csv",
                      na.strings =".", 
                      check.names = FALSE),
     female = read.csv2("varia/archive/input/data/mortality/nonNatural_mortality_byAge_2009_2019_female.csv",
                        na.strings =".", 
                      check.names = FALSE))
...
total_mortality_rawdata <- 
  list(
    male = read.csv2("varia/archive/input/data/mortality/total_mortality_byAge_2009_2019_male.csv",
                     na.strings =".", 
                      check.names = FALSE),
    female = read.csv2("varia/archive/input/data/mortality/total_mortality_byAge_2009_2019_female.csv",
                       na.strings =".", 
                      check.names = FALSE))
...
# Scenario
exp_rawdata <- 
  read.csv("varia/archive/input/data/rr_other_rawdata/exp.csv",
           sep = ";",
           na.strings=c("NA", ""))
...
cutoff_rawdata <- 
  # Read data
  read.csv("varia/archive/input/data/rr_other_rawdata/cutoff.csv",
           sep = ";",
           na.strings=c("NA", ""))
...
social_bimd_rawdata <- 
  read.csv("varia/input/inequalities/BIMD_2011_WITHOUT_HEALTH_ELLIS_WIDE.csv")
...
airqplus_copd_export <-
  ...
  readr::read_delim("varia/archive/input/airqplus_cases/airqplus_COPD.csv",
                    delim = ";",
                    col_types = cols())
...
airqplus_deaths_pop_multipleYear <-
  read.csv2("varia/archive/input/airqplus_cases/airqplus_manual_multipleYear.csv")
airqplus_deaths_pop_singleYear <-
  read.csv2("varia/archive/input/airqplus_cases/airqplus_manual_age0to40_singleYear.csv")
...
airqplus_deaths_yll_export <-
  readr::read_delim(
    # "varia/archive/airqplus_cases/airqplus_deaths_yll_lifetable_adults.csv", # OLD PATH
    "varia/archive/input/airqplus_cases/airqplus_deaths_yll_lifetable_adults.csv",
    delim = ";",
...
## Replace 0 with 1 for age 8 in airqplus_pm_deaths_yll[["pop"]]$number_of_deaths_female
airqplus_pm_deaths_yll[["pop"]]$number_of_deaths_female[9] <- 1 # row 9 corresponds to age 8
```
...
niph_noise_ha_excel <-
  # Read csv file
  readxl::read_xlsx(path = "varia/archive/input/noise_niph/example_road_noise_niph.xlsx",
...
choice_rr_main_selected <- 
  ...
  select(rr_id) %>% unique() %>% dplyr::pull()
choice_rr_main_sensitivity <- 
  rr_data %>% 
  dplyr::filter(pair_id %in% choice_pairs_main & 
                  choice_rr %in% "sensitivity") %>% 
  dplyr::select(rr_id) %>% unique() %>% dplyr::pull()
...
mrbrt_stroke <- readRDS(file = "varia/archive/input/stroke_niph/MRBRT_stroke.RDS")
```
...
for (s in sex){
  population_byAge[[s]] <- 
    ...
    dplyr::select(age, dplyr::starts_with(s)) %>%
    ...
    dplyr::mutate(
      population = rowMeans(dplyr::across(dplyr::contains(s)), na.rm = TRUE)) %>%
...
pop_data_norway <- readRDS(file = "varia/archive/input/stroke_niph/pop_data.RDS")
```
...
for(s in sex){
    ...
    nonNatural_mortality[[s]] <- 
      ...
      dplyr::select(dplyr::all_of(c("Age", as.character(2010:2019)))) %>%
...
exdat_socialize <- dplyr::bind_rows(
  ...
  dplyr::select(NUTS1, CS01012020, age_group, PM25_MEAN, RR, MORTALITY_TOTAL, POPULATION, score) |>
...
Variables that are used in examples of testing_Rpackage
...
```{r modify variable names for package example data}

# exdat_pop_multi <- airqplus_deaths_pop_multipleYear # Avoid using lifetable data where one age groups included more than one year
# exdat_pop <- airqplus_deaths_pop_singleYear # Doesn't seem to be used in testing_Rpackage
exdat_pm_copd <- airqplus_pm_copd
exdat_pop_1 <- airqplus_pm_deaths_yll[["pop"]]
# exdat_input_1 <- airqplus_pm_deaths_yll[["input"]] # DEACTIVATED 2025-08-12 AL neither used in vignettes nor fun doc
# exdat_input_2 <- input_data_mortality # DEACTIVATED 2025-08-12 AL as it's used to provide the exact same values as also available in exdat_input_1
# exdat_pop_male <- lifetable_withPopulation[["male"]] # DEACTIVATED 2025-08-12 AL as all data from here is also contained in exdat_pop_1 (airqplus_pm_deaths_yll[["pop"]]
# exdat_pop_female <- lifetable_withPopulation[["female"]] # DEACTIVATED 2025-08-12 AL as all data from here is also contained in exdat_pop_1 (airqplus_pm_deaths_yll[["pop"]]
# exdat_erf <- mrbrt_stroke # DEACTIVATED 2025-08-12 AL as not used in vignettes nor fun doc
# exdat_noise_ha <- niph_noise_ha_excel |> # DEACTIVATED 2025-08-12 AL new exdat_noise variable will be created
  # dplyr::filter(!is.na(niph_noise_ha_excel$exposure_mean))
# exdat_noise_ihd <- niph_noise_ihd_excel # DEACTIVATED 2025-08-12 AL as exdat_noise_ihd not used in vignette nor fun doc
# exdat_pop_2 <- pop_data_norway # DEACTIVATED 2025-08-12 AL as exdat_pop_2 not used in vignette nor fun doc
exdat_mdi <- social_data
exdat_get_mdi <- read.csv("varia/archive/input/data/mdi/Belgium_MDI_2021.csv")
```

```{r tidy up and rename example data}

exdat_noise <- tibble::tibble(
  exposure_category = c("55-59", "60-64", "65-69", "70-74", "≥ 75"),
  exposure_mean = c(57.5, 62.5, 67.5, 72.5, 77.5),
  pop_exp_urban = c(327900, 236500, 167300,  69400, 7700),
  pop_exp_rural = c(59600, 49500, 24500, 2800, 0),
  pop_exp_total = c(387500, 286000, 191800, 72200, 7700)
) |> 
  tidyr::pivot_longer( cols = dplyr::starts_with("pop_exp_"), names_to = "region", values_to = "exposed" ) |> 
  dplyr::mutate(region = base::strsplit(region, "_") |> purrr::map_chr(\(x) x[3])) |> 
  dplyr::mutate(prop_pop_exp = dplyr::case_when(
    region == "urban" ~ exposed / 4416404, #   total_urban_pop = 4416404,
    region == "rural" ~ exposed / 797581, #   total_rural_pop = 797581,
    region == "total" ~ exposed / 5213985 #   total_norway_pop = 5213985
  ))

exdat_lifetable <- exdat_pop_1 |> 
  ...
  tidyr::pivot_longer( cols = -age_group, names_to = c(".value", "sex"), names_pattern = "(midyear_population|deaths)_(male|female)" ) |> 
  dplyr::arrange(sex)

exdat_pm <- exdat_pm_copd
...
rm(list=setdiff(ls(), 
                c(
                  "exdat_pm", # OLD NAME: "exdat_pm_copd"
                  "exdat_lifetable", # OLD NAME: "exdat_pop_1",
                  # "exdat_input_1",
                  # "exdat_input_2",
                  # "exdat_pop_male",
                  # "exdat_pop_female",
...
```{r save package example data}

## Save variables to RData file
save.image(file = "../healthiar/data/example_data.RData")

# EXPERIMENTAL: save each object as .rds file
# Get the list of all objects in the environment
# objects_in_env <- ls()
# 
# dir_name <- "../healthiar/data"
# 
# # Save each object as an .rds file in the specified directory
# for (obj_name in objects_in_env) {
#   obj <- get(obj_name)
#   file_name <- paste0(dir_name, "/", obj_name, ".rds")
...
#                   "pop_data_norway"
#                   )))
# save.image(file = "varia/archive/output/input_data_for_testing_Rpackage.RData")
```
