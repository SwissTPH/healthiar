...
format:
  revealjs:
    ...
    # code-tools: # Test at later point what this does
      # source: https://github.com/SwissTPH/healthiar/tree/master
...
This presentation was part of an online *healthiar* workshop by [Swiss TPH](https://www.swisstph.ch/en/about/eph/environmental-exposures-and-health) held on 2025-10-01, which was recorded. [**The recording (45') can be found here**](https://team.swisstph.ch/s/aN_wN5MUTAS3bwEkWvtvaQ): https://team.swisstph.ch/s/aN_wN5MUTAS3bwEkWvtvaQ

See the README file of the [*healthiar* GitHub repository for **information on how to install *healthiar* in RStudio**](https://github.com/SwissTPH/healthiar?tab=readme-ov-file#readme) and how to get started: https://github.com/SwissTPH/healthiar?tab=readme-ov-file#readme

Feedback is very welcome via [GitHub issues](https://github.com/SwissTPH/healthiar/issues) (preferred) or by email (addresses on first slide).

Disclaimer: the R package is still a work in progress - it could be that some of the exact code in this presentation will cede to work at some point, but the concepts behind them should stay the same.

The slides of this presentation can be found here: https://github.com/SwissTPH/healthiar/tree/master/varia/Workshops_and_demos/workshop
...
-   [**1**]{style="color: green"} purpose - to help you quantify and monetize the attributable burden of disease

-   [**3**]{style="color: green"} dimensions - health, monetization, social aspects

-   [**20**]{style="color: green"} functions

-   [**> 1000**]{style="color: green"} calculation pathways

-   [**60000**]{style="color: green"} (simple) assessments per second

## `healthiar` overview

![](images/package_overview_1.png){width="1800" fig-align="left"}

## `healthiar` overview

![](images/package_overview_2.png){width="1800" fig-align="left"}

## `healthiar` overview

![](images/package_overview_3.png){width="1800" fig-align="left"}
...
![](images/bod_rr_1.png){fig-align="left" width="1400"}

## Refresher - Burden of disease with relative risk

![](images/bod_rr_2.png){fig-align="left" width="1500"}

## Refresher - Burden of disease with relative risk

![](images/bod_rr_3.png){fig-align="left" width="1500"}

## Refresher - Burden of disease with relative risk

![](images/bod_rr_4.png){fig-align="left" width="1500"}
...
results_pm_copd <- attribute_health(
  erf_shape = exdat_pm$erf_shape,
  rr_central = exdat_pm$relative_risk, 
  rr_increment = exdat_pm$rr_increment, 
...
::: callout-tip
## Interpretation
3'502 cases can be attributed to PM2.5 exposure.
:::
::::
...
```{r, echo=TRUE,eval=FALSE,include=TRUE}
View(results_pm_copd$health_main)
```

::: fragment
![](images/rstudio_env_3.png){width="600" fig-align="left"}
:::
...
::: fragment
Then click on the little scroll symbol

![](images/rstudio_env_2.png){fig-align="left"}
:::

::: fragment
This opens a table of the results

![](images/rstudio_env_3.png){fig-align="left"}
:::
:::
...
::: callout-tip
## Interpretation
By implementing the clean air policy we expect that 2'180 COPD cases can be avoided.
:::
::::
...
:::: fragment
::: callout-tip
The main results contain results aggregated by `geo_id_macro` (if available).
:::
::::
...
:::: fragment
::: callout-tip
Impact per geo unit specified in `geo_id_micro` is stored in the detailed results.
:::
::::
...
results_mr_brt <- attribute_health(
  exp_central = 8.85,
  bhd_central = 30747,
...
```{r, echo=FALSE,include=TRUE,eval=TRUE}
results_mr_brt[["health_main"]] |> 
  dplyr::select(impact_rounded, impact, pop_fraction, erf_ci, rr_at_exp, exp, bhd) |> 
  dplyr::slice(1) |> 
  knitr::kable()
```
:::
...
```{r, echo=FALSE,include=TRUE,eval=TRUE}
results_pm_copd[["health_main"]] |> 
  dplyr::select(impact_rounded, pop_fraction, exp, prop_pop_exp, rr_at_exp) |> 
  dplyr::slice(1) |> 
  knitr::kable()
```
:::
...
![](images/bod_ar_1.png){fig-align="left" width="1500"}

## Refresher - Burden of disease with absolute risk

![](images/bod_ar_2.png){fig-align="left" width="1500"}
...
-   `daly()` sums attributable YLL and YLD impacts
...
-   `prepare_exposure()` calculates (population-weighted) mean exposure by combining spatial exposure and geographic units data 
...
-   `healthiar` is publicly available on this [GitHub page](https://github.com/SwissTPH/healthiar)
...
# Second hour programme

## Second hour programme

- Exercises in small groups (ca. 40 min)
...
::: fragment
**Exercises** and **slides** from this presentation can be downloaded from [this folder on the *healthiar* GitHub page](https://github.com/SwissTPH/healthiar/tree/master/varia/Workshops_and_demos/workshop) (link also in the last email and in the Zoom chat).
:::


::: fragment
**Install and load the `healthiar` R package** (code also found at the start of the exercises). 
...
Please answer 3 short questions to provide your valuable feedback via [this short form](https://swisstph.wufoo.com/forms/s1k7txho0lrkpj1/) (link also in the last email and in the Zoom chat)

Also, any input via [GitHub issues](https://github.com/SwissTPH/healthiar/issues) is also welcome.

Or you can write to Alberto (alberto.castrofernandez\@swisstph.ch) and Axel (axel.luyten\@swisstph.ch).
