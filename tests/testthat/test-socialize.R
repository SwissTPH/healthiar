...
testthat::test_that("results correct |pathway_socialize|input_is_attribute_output_TRUE|social_indicator_TRUE|ref_pop_TRUE|", {
  ...
  pop_ref <- readRDS(testthat::test_path("testdata", "pop_ref.rds"))
  no2_mrt_mdi <- readRDS(testthat::test_path("testdata", "no2_mrt_mdi.rds"))
  ...
  testthat::expect_equal(
    object =
      healthiar::socialize(
        ...
        )$social_main$difference_value[1:4] |> round(3),
  ...
  testthat::expect_equal(
    ...
    object =
      healthiar::socialize(
        ...
        geo_id_micro = unique(data$SECTOR), # geo IDs of the preparatory iteration call above and this function call must match!
  ...
  testthat::expect_equal(
    ...
    object =
      healthiar::socialize(
        ...
        geo_id_micro = data$SECTOR, # geo IDs of the preparatory iteration call above and this function call must match!
        # social_indicator = data$SCORE,
        social_quantile = as.numeric(gsub("D", "", data$DECILE)),
        # n_quantile = 10, # Specify number of quantiles, e.g. 10
        population = data$POP,
        age_group = data$AGE,
        ref_prop_pop = data$REF
...
testthat::test_that("error if non-numeric", {
  ...
  testthat::expect_error(
    ...
    object =
      healthiar::socialize(
        impact = as.character(data$IMPACT), #
        geo_id_micro = data$SECTOR,
        social_indicator = data$SCORE,
        n_quantile = 10,
        population = data$POP,
...
testthat::test_that("error if non-numeric in numeric var", {
  ...
  testthat::expect_error(
    ...
    object =
      healthiar::socialize(
        impact = as.character(data$IMPACT), # As character to force error
        geo_id_micro = data$SECTOR,
        social_indicator = data$SCORE,
        n_quantile = 10,
        population = data$POP,
...
testthat::test_that("warning and no least deprived quantile if social_indicator is NA", {
  ...
  output_socialize <- suppressWarnings(call_socialize())
...
testthat::test_that("error if main_results_by and socialize", {
  ...
  output_attribute <-
    healthiar::attribute_health(
      info = data.frame(pair = rep(c("copd", "asthma"), each = 4)),
      main_results_by = "pair",
      age_group = rep(c("below_40", "above_40"), times = 4),
      geo_id_micro = rep(rep(c("g1", "g2"), each = 2), times = 2),
      exp_central = rep(c(8.1, 22.1), each = 4),
      cutoff_central = 0,
      bhd_central = rep(c(1000, 4000), times = 4),
      rr_central = rep(c(1.063, 1.041), each = 4),
      rr_increment = 10,
      erf_shape = "log_linear",
      population = rep(c(1E5, 5E5), times = 4))
