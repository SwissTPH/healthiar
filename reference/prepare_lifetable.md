...
including the last one. An open-ended top age group such as "90 years
old and over" therefore has to be entered as the closed age interval
that follows the age groups below it, here \[90, 95). Its hazard rate is
then the one of the whole open-ended group, which is higher than the
hazard rate of a closed age interval at that age, and the deaths of the
ages above the interval are placed inside it.
