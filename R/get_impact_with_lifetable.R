...
get_impact_with_lifetable <-
  function(input_with_risk_and_pop_fraction){
    ...
    is_single_year_exposure <- unique(input_with_risk_and_pop_fraction$approach_exposure) == "single_year"
    is_constant_exposure <- unique(input_with_risk_and_pop_fraction$approach_exposure) == "constant"

    is_with_newborns <- unique(input_with_risk_and_pop_fraction$approach_newborns) == "with_newborns"


    # Store the value of time horizon (defined in compile_input())
    time_horizon <- unique(input_with_risk_and_pop_fraction$time_horizon)
    ...
    years_projection <- yoa + seq_len(time_horizon - 1)
    # n_years_projection defines for how many years the population should be projected;
    n_years_projection <- length(years_projection)

    # Precompute column names to be use below
    entry_names <- paste0("entry_population_", years_projection)
    midyear_names   <- paste0("midyear_population_", years_projection)
    death_names <- paste0("deaths_", years_projection)
    ...
    project_yoa <- function(df, prob_survival, prob_survival_until_midyear = NULL){
      ...
      if (!is.null(prob_survival_until_midyear)) {
        df$midyear_population_yoa <-
          round(df$entry_population_yoa * prob_survival_until_midyear, 10)
      }

      # End-of-year population YOA = (entry population YOA) * (survival probability)
      df$end_population_yoa <-
        round(df$entry_population_yoa * prob_survival, 10)
    ...
    if (is_yll || #And  ("yld", "daly") if yld for life table ever implemented
      ...
      project_pop <- function(df, prob_survival, prob_survival_until_midyear) {
        ...
        names(df) <- names(df) |>
          ...
          gsub("_yoa_plus_1", paste0("_", yoa_plus_1), x = _) |>
          gsub("_yoa", paste0("_", yoa), x = _)
        ...
        # Initialise matrices
        entry_pop <- matrix(NA, nrow = n_ages, ncol = n_years_projection,
                                  # Row and column names
                                  # NULL because no row names
                                  dimnames = list(NULL, entry_names))
        midyear_pop <- matrix(NA, nrow = n_ages, ncol = n_years_projection,
                                  dimnames = list(NULL, midyear_names))
        deaths <- matrix(NA, nrow = n_ages, ncol = n_years_projection,
                                  dimnames = list(NULL, death_names))      
        

        # Set initial year
        entry_pop[, 1] <- df[[entry_names[1]]]
        midyear_pop[, 1] <- round(entry_pop[, 1] * prob_survival_until_midyear, 10)
        deaths[, 1] <- round(entry_pop[, 1] * death_prob, 10)
        ...
        for (i in seq_len(n_years_projection - 1)) {
          ...
          entry_pop[rows, i + 1] <- round(entry_pop[rows - 1, i] * prob_survival[rows - 1], 10)
          # MID-YEAR POP YOA+1 <- ( ENTRY POP YOA+1) * ( SURVIVAL PROBABILITY FROM START OF YOA+1 TO MID YEAR YOA+1)
          midyear_pop[rows, i + 1]   <- round(entry_pop[rows, i + 1] * prob_survival_until_midyear[rows], 10)
          # DEATHS IN YOA+1 <- ( ENTRY POP YOA+1 ) * (1 - SURVIVAL PROBABILITY YOA+1 )
          deaths[rows, i + 1]    <- round(entry_pop[rows, i + 1] * death_prob[rows], 10)
        }
      ...
      var_prefix_for_function <-
        ifelse(is_deaths, "deaths_", "midyear_population_")
      ...
      if (is_with_newborns && is_constant_exposure) {
        ...
        fill_right_of_diag <- function(tbl) {
          ...
          cols <- setdiff(names(tbl), c("age_start", "age_end", "population"))
          data_selection <- tbl[, cols, drop = FALSE]

          n_years <- ncol(data_selection)
          ...
          for (i in seq_len(min(nrow(data_selection),
                                            n_years - 1))) {
          ...
          function(.x){
            ...
            .x <-
              ...
              dplyr::mutate(.by = c(age_start, age_end),
                            population = ifelse(year == as.character(yoa),
                                                      population,
                                                      NA))
            ...
            if(is_deaths && is_single_year_exposure){
              .x <- .x |>
                ...
                dplyr::filter(year == as.character(yoa))

            } else {
              .x <- .x |>
                ## Select all years within time horizon
                dplyr::filter(as.numeric(year) <= last_year_projection)
            }
          }
